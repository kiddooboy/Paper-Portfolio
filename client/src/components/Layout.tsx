...
export default function Layout() {
  ...
  return (
    <div className={cn('min-h-screen bg-gray-50 dark:bg-groww-dark text-gray-900 dark:text-groww-text transition-colors', dark ? 'dark' : '')}>
      ...
      <div className="flex">
        <aside className="hidden lg:flex flex-col w-60 shrink-0 sticky top-[60px] h-[calc(100vh-60px)] border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-groww-dark">
          <div className="flex-1 overflow-y-auto">
            <Sidebar activePath={location.pathname} />
          </div>
          {/* AI Chat pinned to sidebar bottom */}
      ...
      <button
        onClick={() => navigate('/ai-chat')}
        className="lg:hidden fixed bottom-20 right-4 z-50 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 focus:outline-none"
        title="AI Assistant"
...
function IndexTicker() {
  ...
  return (
    <div className="w-full bg-white dark:bg-groww-dark border-b border-gray-200 dark:border-gray-800">
      <div className="grid divide-x divide-gray-200 dark:divide-gray-800" style={{ gridTemplateColumns: `240px repeat(${items.length - 1}, 1fr)` }}>
