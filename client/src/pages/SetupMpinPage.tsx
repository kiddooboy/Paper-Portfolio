              ...
              <div className="flex justify-center gap-2">
                {[0, 1, 2, 3].map((i) => (
                  <div
                    ...
                    className={cn(
                      'w-14 h-14 rounded-lg border-2 flex items-center justify-center text-2xl font-bold transition-all',
                      confirmMpin[i] ? 'border-green-500 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400' : 'border-gray-300 dark:border-gray-700'
                    )}
                  >
                    {showMpin ? confirmMpin[i] : confirmMpin[i] ? '●' : ''}
                  </div>
                ))}
              </div>
              {confirmMpin && confirmMpin !== mpin && (
